...
const ImageUploader = () => {
  ...
  const handleCapture = async () => {
    if (cameraPermission) {
      ...
      //console.log(capturedImage1['uri'])
      let img= capturedImage1['uri']
      console.log(img)
  ...
  const handleImageUpload = async () => {
    if (capturedImage) {
      const apiUrl = 'https://c4a9-197-251-195-80.ngrok-free.app/upload-image';
      ...
      try {
        ...
        setReturnObj(response.data)
        Speech.speak("There is "+response.data+" infront of you");
  ...
  return (
    <View>
      ...
      {capturedImage && (
        <View>
          <Image
            source={{ uri: capturedImage1.uri }}
            style={{  display: 'flex', marginLeft: 56, width: 300, height: 200 }}
          />
